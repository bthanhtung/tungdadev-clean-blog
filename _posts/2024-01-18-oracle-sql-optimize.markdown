...
Ở các dự án lớn, quy mô lớn thường có cả một đội DBA chuyên nghiệp làm công tác tối ưu SQL, nhưng đôi khi dự án cần release gấp với lượng người dùng ban đầu hạn chế thì chính dev cũng làm luôn công việc của DBA. Nếu bạn đã từng bị stress vì những câu truy vấn chậm chạp như rùa, thì đây chính là bài viết dành cho bạn.
 ...
 Hãy chuẩn bị sẵn sàng vì bạn sẽ không chỉ học được cách tối ưu mà còn được ... được cái gì nữa thì cũng méo biết!

## 1. Tại sao phải tối ưu SQL Query?
...
## 2. Hiểu rõ về dữ liệu của bạn

Trước khi làm bất cứ điều gì, bạn cần phải hiểu rõ về dữ liệu của mình. Hãy tưởng tượng bạn là một thám tử, và dữ liệu của bạn chính là vụ án cần được giải quyết. Dưới đây là vài điều cần chú ý:
...
- **Phân phối dữ liệu**: Nếu bạn có một bảng lớn với hàng triệu bản ghi, hãy chắc chắn rằng bạn biết dữ liệu trong đó được phân phối như thế nào. Hãy tìm ra những bản ghi `“hot hòn họt”` và những bản ghi `“lạnh băng giá”` để biết nên tập trung vào đâu.

Giả sử bạn có một bảng `customers` với hàng chục triệu bản ghi. Nếu bạn chỉ cần tìm kiếm khách hàng ở một thành phố cụ thể, hãy chắc chắn rằng bạn biết cách nhóm và chỉ định các bản ghi này.
...
### 3. Một số cách tối ưu SQL Query 
##### 3.1. Sử dụng Index
...
- Khi bạn thường xuyên tìm kiếm trên một cột.
...
Đừng bao giờ sử dụng `SELECT *`. Điều này giống như việc bạn gọi món ăn mà không biết mình đang ăn gì. Hãy chỉ chọn những cột mà bạn thực sự cần. Điều này không chỉ giúp tăng tốc độ truy vấn mà còn tiết kiệm tài nguyên.
...
Hãy đảm bảo rằng bạn không sử dụng quá nhiều `JOIN` trong một truy vấn. Điều này giống như việc bạn mời quá nhiều bạn bè đến bữa tiệc – thật hỗn độn!

##### 3.4. Sử dụng EXPLAIN PLAN
Trước khi chạy một truy vấn lớn, hãy sử dụng `EXPLAIN PLAN` để hiểu cách Oracle xử lý truy vấn của bạn. Điều này sẽ giúp bạn xác định xem có bất kỳ nút thắt cổ chai nào trong truy vấn hay không.
...
Điều này giống như việc bạn xem xét kế hoạch du lịch trước khi đi. Bạn cần biết mình sẽ đi đâu và làm gì để không bị lạc đường!

##### 3.5. Tránh sử dụng các hàm trong WHERE clause
Khi bạn sử dụng hàm trong câu lệnh `WHERE`, điều này có thể khiến Oracle không sử dụng chỉ mục một cách hiệu quả. Hãy tránh điều này nếu có thể.
...
Điều này sẽ giúp bạn tiết kiệm thời gian như việc không cần đợi đến khi người khác quyết định chọn món ăn!

##### 3.6. Tối ưu hóa các câu truy vấn con
...
Điều này sẽ giúp bạn tối ưu hóa hiệu suất như việc bạn chuyển từ ăn bún phở sang sushi – ăn ít nhưng vẫn no!

##### 3.7. Sử dụng Partitioning
Nếu bạn có bảng lớn, hãy xem xét việc phân vùng bảng `(table partitioning)`. Điều này sẽ giúp Oracle chỉ quét qua những phần cần thiết của bảng thay vì toàn bộ bảng.
...
##### 3.8. Kiểm tra hiệu suất thường xuyên
